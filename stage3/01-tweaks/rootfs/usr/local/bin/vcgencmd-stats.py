...
topic_json = hostname + '/sensors/' + name.upper() + '/temperature'
...
pmic_temp_str = check_output(['vcgencmd', 'measure_temp', 'pmic']).decode('utf-8').strip()
use_pmic = not pmic_temp_str.startswith('error')
if use_pmic:
  print('pmic temp sensor found')
else:
  print('pmic temp sensor not found')
...
while True:
  ...
  gpu_temp_str = check_output(['vcgencmd', 'measure_temp']).decode('utf-8').strip()
  gpu_temp = float(re.split("[=']", gpu_temp_str)[1])
  values = {
      "gpu_degC": gpu_temp,
      }
  ...
  if use_pmic:
    pmic_temp_str = check_output(['vcgencmd', 'measure_temp', 'pmic']).decode('utf-8').strip()
    pmic_temp = float(re.split("[=']", pmic_temp_str)[1])
    values["pmic_degC"] = pmic_temp
  ...
  cpu_voltage_str = check_output(['vcgencmd', 'measure_volts', 'core']).decode('utf-8').strip()
  cpu_voltage = re.split("[=V]", cpu_voltage_str)[1].rstrip('0')
  values["voltage_V"] = float(cpu_voltage)
